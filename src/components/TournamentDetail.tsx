...
export default function TournamentDetail({ tournament }: Props) {
  ...
  // MCR tournaments: show bookings grid directly (same as MCR View)
  if (isMCR) {
    return (
      <div className="flex flex-col h-full overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-card shrink-0">
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-bold">{tournament.name}</h2>
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border bg-blue-500/15 text-blue-400 border-blue-500/30">MCR</span>
          </div>
  ...
  // Non-MCR tournaments: full metadata + bookings
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Scrollable metadata */}
      <div className="p-6 space-y-6 max-w-3xl shrink-0 overflow-auto">
        {/* Header */}
      ...
      {/* Bookings — takes remaining space */}
      <div className="flex-1 min-h-[300px] flex flex-col border-t border-border">
        <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider px-4 pt-3 pb-2 shrink-0">
          Bookings
        </h3>
        <div className="flex-1">
          <BookingsGrid tournamentId={tournament.id} hideViewToggle />
